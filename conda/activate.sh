...
then
  ...
  then
    ...
    ${ZOWE_ROOT}/components/app-server/share/zlux-app-server/bin/install-app.sh $PREFIX/opt/zowe/plugins/app-server/$PKG_NAME/$PKG_VERSION
  fi
else
  ${ZOWE_INST}/bin/install-app.sh $PREFIX/opt/zowe/plugins/app-server/$PKG_NAME/$PKG_VERSION
fi
