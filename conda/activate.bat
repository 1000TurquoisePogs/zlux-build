...
if not defined ZOWE_INST (
  if not defined ZOWE_ROOT (
    ...
    !ZOWE_ROOT!\components\app-server\share\zlux-app-server\bin\install-app.bat %PREFIX%\opt\zowe\plugins\app-server\%PKG_NAME%\%PKG_VERSION%
  )
) else (
  !ZOWE_INST!\bin\install-app.bat %PREFIX%\opt\zowe\plugins\app-server\%PKG_NAME%\%PKG_VERSION%
)
